...
export async function Router({ ROOT, app }: {
  ...
  await Promise.all(
    fileArr.map(async (filePath) => await Require.default(filePath)),
  );
    ...
    for (const [methodName, methodInfo] of methodMap) {
      ...
      paths!.forEach(({ path: p, methodTypes }) => {
        ...
        if (RoutePathSet.has(routePath)) {
          return console.log(`[URL NOT UNIQUE] ${routePath}`);
        }
        ...
        if (p!.indexOf(":") > -1 || p!.indexOf("(") > -1) {
          ...
          RegexpRouterMap.set(
            pathReg,
            { clazz, keys, routePath, methodTypes, methodName },
          );
        } else {
          StaticRouterMap.set(
            routePath,
            { clazz, routePath, methodTypes, methodName },
          );
        }
      });
    }
  ...
  function MatchRegexp(reqPath: string) {
    for (
      const [reg, { clazz, methodName, keys, methodTypes }] of RegexpRouterMap
    ) {
  ...
  return (ctx: IContext, next: Function) => {
    ...
    if (
      staticResult &&
      (!staticResult.methodTypes ||
        staticResult.methodTypes.indexOf(methodType) > -1)
    ) {
    ...
    if (
      regexpResult &&
      (!regexpResult.methodTypes ||
        regexpResult.methodTypes.indexOf(methodType) > -1)
    ) {
