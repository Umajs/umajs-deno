...
export async function callMethod(
  ...
  const methodResult = await Promise.resolve(
    Reflect.apply(method, instance, argsV),
  );

  if (methodResult instanceof Result) return Result.finish(ctx, methodResult);

  throw new Error(
    `[NOT_RETURN_RESULT] ${name}.${methodName} does not return result, e.g "Result.[view|json]()"`,
  );
}
